...
void q15_axpy_rvv(const int16_t *a, const int16_t *b,
                  int16_t *y, int n, int16_t alpha)
{
#if !defined(__riscv) || !defined(__riscv_vector) || (__riscv_v_intrinsic < 11000)
    ...
    for (; n > 0; n -= vl, a += vl, b += vl, y += vl) {
        ...
        vint32m8_t v_prod = __riscv_vwmul_vx_i32m8(vb, alpha, vl);
        vint32m8_t v_acc = __riscv_vwadd_wv_i32m8(v_prod, va, vl);
        vint16m4_t v_res = __riscv_vnclip_wx_i16m4(v_acc, 0, vl);
